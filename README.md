...
3. New review on the PR with state `CHANGES_REQUESTED` (or, with
   ...
   branch, refreshes the PR title and description, replies to each inline
   thread and resolves the ones it fully addressed (leaving the rest for
   you), and summarizes in a PR comment; the daemon then requests a fresh
